...
### 7.3 낙관 vs 정식 (발표 프레이밍)
| | dev 내부분할(낙관) | 정식 train→dev(신뢰) |
|---|---:|---:|
| full Utt-EER | 0.84 | **2.22** |
| full Range-EER | 7.69 | **7.93** |

> 정식 프로토콜에서 **탐지 0.84%→2.22%로 현실화**(여전히 딥 SOTA 0.5~4% 범위), 국소화는 7.93%로 거의 동일 → 견고. "낙관적 수치 + 정식 수치 병기"가 정직성.
